...
export default async function PracticeAreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 py-16">
      ...
      <ul className="flex flex-col gap-3 rounded-md border border-stone-200 bg-white p-6">
        {area.items.map((item) => (
      ...
      <Link
        href="/contact"
        className="w-fit rounded-md bg-stone-900 px-5 py-3 font-medium text-amber-50 transition-colors hover:bg-stone-800"
      >
        Discuss your matter
      </Link>
    </div>
